...
import { costCaption, formatTokens, formatUsd, usageChip, reportedCost } from "@/lib/usage";
import {
  useStore,
  useStreaming,
  formatTime,
...
function UsageChip({ bot }: { bot: Bot }) {
  ...
  const detail = [
    ...
    reportedCost(usage.costUsd) ? `${formatUsd(usage.costUsd)} ${costCaption(billing)}` : null,
  ]
    .filter(Boolean)
    .join("\n");
  return (
